...
class MainViewModel : ViewModel() {
    ...
    var data = mutableStateOf(emptyList<Decoration>())
        private set
    ...
    fun saveData(
        ...
        tipe_dekorasi: String,
        harga: String,
        bitmap: Bitmap
    ) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val part1 = RequestBody.create("text/plain".toMediaTypeOrNull(), nama_dekorasi)
                val part2 = RequestBody.create("text/plain".toMediaTypeOrNull(), tipe_dekorasi)
                val part3 = RequestBody.create("text/plain".toMediaTypeOrNull(), harga)
